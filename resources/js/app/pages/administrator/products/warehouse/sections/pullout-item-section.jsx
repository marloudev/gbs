...
import { get_receives_thunk, get_warehouses_thunk, pullout_item_thunk } from '../../redux/products-thunk';
...
export default function PulloutItemSection({ data }) {
    ...
    const remaining = isNaN(data.quantity - (form?.pullout_count ?? 0)) ? 0 : data.quantity - (form?.pullout_count ?? 0);
    async function pullout_item(e) {
        e.preventDefault()
        setLoading(true)
        try {
            await store.dispatch(pullout_item_thunk({
                ...data,
                ...form,
    ...
    return (
        <div>
            <Button variant='contained' onClick={handleOpen}>PULLOUT</Button>
            <Modal
                open={open}
                onClose={handleClose}
            ...
            >
                <Box sx={style}>
                    <div
                    >
                        <Typography id="modal-modal-title" className='py-5' variant="h6" component="h2">
                        ...
                        <Table aria-label="simple table">
                            <TableHead>
                                ...
                                <TableRow>
                                    ...
                                    <TableCell align="right">{isNaN(form?.pullout_count ?? 0) ? 0 : form?.pullout_count ?? 0}</TableCell>
                                </TableRow>
                                <TableRow>
                        ...
                        <div className='w-full flex items-end justify-end mt-12 gap-5'>
                            ...
                            >Cancel</Button>
                            <Button
                                disabled={(remaining < 0) || form.pullout_count == "" || loading}
                                variant='contained'
